...
class RankingServiceTest {
	...
	@Test
	@DisplayName("Debería registrar participante exitosamente cuando el torneo existe y no está duplicado")
	void registrarParticipanteExitoso() {
		when(tournamentClient.obtenerResumen(100L)).thenReturn(torneoResumenMock);
		...
		assertEquals(100L, response.getTorneoId());
		assertEquals(50L, response.getParticipanteId());

		verify(tournamentClient, times(1)).obtenerResumen(100L);
		verify(rankingRepository, times(1)).save(any(Ranking.class));
	}

	@Test
	@DisplayName("Debería lanzar IllegalStateException al registrar si el torneo no existe")
	void registrarParticipanteTorneoNoExiste() {
		when(tournamentClient.obtenerResumen(100L)).thenThrow(new RuntimeException("Feign error 404"));

		IllegalStateException exception = assertThrows(IllegalStateException.class, () -> {
			rankingService.registrarParticipante(requestRegistro);
		});

		assertEquals("El torneo especificado no existe.", exception.getMessage());
		verify(rankingRepository, never()).save(any(Ranking.class));
	}

	@Test
	@DisplayName("Debería lanzar IllegalStateException si el participante ya está inscrito en el torneo")
	void registrarParticipanteDuplicado() {
		when(tournamentClient.obtenerResumen(100L)).thenReturn(torneoResumenMock);
		when(rankingRepository.existsByTorneoIdAndParticipanteId(100L, 50L)).thenReturn(true);

		IllegalStateException exception = assertThrows(IllegalStateException.class, () -> {
			rankingService.registrarParticipante(requestRegistro);
		});

		assertEquals("El participante ya está en el torneo.", exception.getMessage());
		verify(rankingRepository, never()).save(any(Ranking.class));
	}

	@Test
	@DisplayName("Debería retornar la lista vacía o con datos al solicitar la tabla de posiciones")
	void obtenerTablaPosiciones() {
		...
		assertEquals(1, tabla.getFirst().getPosicion()); // Uso de .getFirst() nativo de Java 21
	}

	@Test
	@DisplayName("Debería actualizar estadísticas y recalcular posiciones al procesar un resultado")
	void actualizarConResultadoExitoso() {
		...
		Ranking rGanador = Ranking.builder()
				.id(ganadorId)
				.torneoId(torneoId)
		...
		Ranking rPerdedor = Ranking.builder()
				.id(perdedorId)
				.torneoId(torneoId)
				.participanteId(perdedorId)
				.puntos(0)
				.victorias(0)
				.derrotas(0)
				.diferencia(0)
		...
		assertEquals(1, rGanador.getVictorias());

		assertEquals(0, rPerdedor.getPuntos());
		assertEquals(-6, rPerdedor.getDiferencia());
		assertEquals(1, rPerdedor.getDerrotas());

		verify(rankingRepository, times(1)).save(rGanador);
		verify(rankingRepository, times(1)).save(rPerdedor);
